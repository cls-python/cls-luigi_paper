...
# Downloads the dataset from OpneML and executes the pipelines
python main.py -- ds_id 9957 # dataset qsar-biodeg
python main.py --ds_id 359958 # dataset pc4
 ...
 ...
````
To replicate the results in the paper, run `python.main` for all dataset IDs:
...
While directories **with** "incumbent" in the name contain the outputs of the test phase with the best pipeline:
...
Note that datasets are downloaded automatically from OpenML and split as follows:
...
### Logs
After running all pipelines, you can find in the `binary_classfication_pipelines/logs/` directory files about:
...
- Allocated time for Auto-Sklearn is decided based on the time taken to run the binary classification pipelines in CLS-Luigi (per dataset)
- Datasets are downloaded and split only when running the binary classification pipelines.
...
Check out the Jupyter Notebooks `askl_vs_cls-luigi.ipynb` and  `shortest_path_pipelines_analysis.ipynb` to see how we calculated time and run saving using Luigi's caching mechanism.

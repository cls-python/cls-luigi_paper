...
from models.pyepo_gurobi_shortest_path_solver import PyEPOGurobiShortestPathSolver
from shortest_path_pipelines.autogluon_multi_label_predictor import MultilabelPredictor
from torch_dataset import TorchDataset
from models.nn_regressor import fcNet
from utils.time_recorder import TimeRecorder
...
class AutoGluonMultilabelPredictor(SKLMultiOutputRegressionModel):
    abstract = False
    ...
    def run(self):
        ...
        with TimeRecorder(self.output()["run_time"].path):
            ...
            labels = [f"label_{i}" for i in range(0, y_train.shape[1])]

            train = pd.DataFrame(
                columns=[f"feat_{i}" for i in range(0, x_train.shape[1])],
                data=x_train)

            train[labels] = y_train

            x_test = pd.DataFrame(
                columns=[f"feat_{i}" for i in range(0, x_test.shape[1])],
                data=x_test)

            from os.path import join as pjoin
            output_folder = pjoin(RESULTS_PATH, self.global_params.dataset_name) + "/ag/"




            problem_types = ["regression" for i in range(0, y_train.shape[1])]
            eval_metrics = ['mean_squared_error' for i in range(0, y_train.shape[1])]

            print(f"Training {self.__class__.__name__}...")
            multi_predictor = MultilabelPredictor(labels=labels, problem_types=problem_types, eval_metrics=eval_metrics,
                                                  path=output_folder)
            multi_predictor.fit(train, time_limit=time_limit[self.global_params.num_data])


            print(f"Predicting {self.__class__.__name__}...")
            predictions = multi_predictor.predict(x_test)

            test_predictions = predictions[labels].to_numpy()
